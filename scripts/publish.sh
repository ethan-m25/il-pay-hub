...
if git diff --cached --quiet; then
  notify_discord "ℹ️ IL Pay Hub [$TODAY]: no changes ($ACTIVE_COUNT active)"
  exit 0
fi

git commit -m "data: IL daily update $TODAY (+$NEW_TODAY new, $ACTIVE_COUNT active)"
git push origin main

export PATH="/Users/clawii/.npm-global/bin:$PATH"
wrangler pages deploy . --project-name il-payhub --branch main 2>&1 | tail -3 || true

notify_discord "✅ Illinois Pay Hub updated [$TODAY]
