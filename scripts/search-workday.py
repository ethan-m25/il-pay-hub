#!/usr/bin/env python3
"""
il-pay-hub/scripts/search-workday.py
Workday API scraper — Illinois edition.

Illinois Equal Pay Act amendments (effective Jan 1, 2025)

Strategy:
  1. Tenant discovery — Exa search finds *.myworkdayjobs.com URLs → extract Illinois employers
  2. CXS JSON API   — paginate all jobs per employer, filter Illinois (fast, no JS)
  3. HTML job page  — salary in raw HTML; pure regex, no LLM

Run: python3 ~/il-pay-hub/scripts/search-workday.py
"""
...
LOG_FILE      = os.path.expanduser("~/il-pay-hub/scripts/workday.log")
LOCK_FILE     = os.path.expanduser("~/il-pay-hub/scripts/.workday.lock")
LOOKBACK_DATE = (date.today() - timedelta(days=60)).isoformat() + "T00:00:00.000Z"
LARGE_TENANT_THRESHOLD = 500
REGION_SEARCH_TEXT = "Illinois"
...
# ── Seed tenants ─────────────────────────────────────────────────────────────
SEED_TENANTS = [
    ("unitedairlines.wd5.myworkdayjobs.com", "unitedairlines", "United-Airlines-Jobs", "United Airlines"),
    ("mcd.wd5.myworkdayjobs.com", "mcd", "External", "McDonald's"),
    ("walgreens.wd5.myworkdayjobs.com", "walgreens", "External", "Walgreens"),
    ("exelon.wd5.myworkdayjobs.com", "exelon", "External", "Exelon"),
    ("aon.wd1.myworkdayjobs.com", "aon", "AonCareers", "Aon"),
    ("cmegroup.wd1.myworkdayjobs.com", "cmegroup", "External", "CME Group"),
    ("motorolasolutions.wd5.myworkdayjobs.com", "motorolasolutions", "External", "Motorola Solutions"),
    ("abbvie.wd5.myworkdayjobs.com", "abbvie", "External", "AbbVie"),
    ("abbott.wd5.myworkdayjobs.com", "abbott", "Abbott_External", "Abbott"),
    ("baxter.wd5.myworkdayjobs.com", "baxter", "External", "Baxter International"),
    ("allstate.wd5.myworkdayjobs.com", "allstate", "External", "Allstate"),
...
DISCOVERY_QUERIES = [
    'site:myworkdayjobs.com "Illinois" salary 2026',
    'site:myworkdayjobs.com "Chicago" salary "$" 2026',
    'site:myworkdayjobs.com "Chicago" tech OR finance OR healthcare salary 2026',
    'site:myworkdayjobs.com "Naperville" OR "Schaumburg" OR "Deerfield" salary 2026',
    'site:myworkdayjobs.com Illinois pharma OR defense OR manufacturing salary 2026',
]
...
def is_illinois(locations_text, external_path=""):
    """Return True only if the job is plausibly located in Illinois.
...
def main():
    ...
    for host, company_id, tenant, company_name in all_tenants:
        ...
        while offset // limit < max_pages:
            ...
            for p in postings:
                if is_illinois(p.get("locationsText", ""), p.get("externalPath", "")):
                    il_jobs.append(p)
            offset += limit
        ...
        log(f"  Illinois jobs: {len(il_jobs)}")

        # Fetch HTML for each BC job and extract salary
        for i, posting in enumerate(il_jobs, 1):
    ...
    for index, (url, meta) in enumerate(fallback_candidates, 1):
        ...
        if not is_illinois(location, external_path):
            continue
    ...
    for sitemap_url, sitemap_company in SITEMAP_SOURCES:
        ...
        if not job_urls:
            ...
            continue
        il_urls = [u for u in job_urls if any(t in u.lower() for t in
                   ["chicago", "illinois", "-il-", "naperville", "schaumburg",
                    "aurora-illinois", "deerfield", "elgin"])]
        if not il_urls:
            il_urls = job_urls
        log(f"    → {len(job_urls)} total, {len(il_urls)} plausibly IL — checking each")
        for job_url in il_urls[:50]:
            ...
            if not is_illinois(location, ext_path):
                continue
            val_min, val_max = salary
            job = {
                "role": title,
