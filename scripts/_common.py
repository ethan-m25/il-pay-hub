#!/usr/bin/env python3
"""
il-pay-hub/scripts/_common.py
Shared utilities for all IL Pay Hub search scripts.

Illinois Equal Pay Act (amended), 820 ILCS 112/10 et seq.
Effective Jan 1, 2025: employers with 15+ employees must include
pay scale or salary range + description of benefits in all job postings.
"""
...
OUTPUT_FILE = os.path.join(SHARED_DIR, f"il-jobs-raw-{TODAY}.txt")
DATA_FILE   = os.path.expanduser("~/il-pay-hub/data/jobs.json")
...
IL_TERMS = [
    "illinois", "chicago", "evanston", "schaumburg", "naperville",
    "rockford", "joliet", "waukegan", "elgin", "peoria",
    "aurora, il", "aurora il",
    "bloomington, il", "bloomington il",
    "springfield, il", "springfield il",
    "champaign", "decatur", "downers grove", "oak brook",
    "oak park", "lisle", "rosemont", "lincolnshire",
    "lake forest, il", "lake forest il",
    "skokie", "evanston", "cicero", "arlington heights",
    "buffalo grove", "wheaton",
    ", il", "il,",
    "chicagoland", "the loop", "wicker park", "lincoln park",
    "west loop", "river north",
]

# Locations that must NOT be in Illinois jobs
_NON_IL_LOC_TERMS = [
    # Canada
    "toronto", "vancouver", "ontario, canada", "british columbia",
    "alberta, canada", "montreal", "calgary", "ottawa", "edmonton",
    ", canada", "canada,", "remote - canada",
    # Washington DC (separate site)
    "washington, dc", "washington dc", "district of columbia",
    # Colorado (separate site)
    "denver, co", "boulder, co", "colorado springs", "fort collins, co",
    "lakewood, co", "aurora, co",
    # Washington State (separate site)
    "seattle, wa", "bellevue, wa", "kirkland, wa", "redmond, wa",
    "tacoma, wa", "spokane, wa", "washington state",
    # New York (separate site)
    "new york city", "new york, ny", "manhattan, ny", "brooklyn, ny",
    "queens, ny", "bronx, ny", "new york, new york",
    # California (separate site)
    "san francisco, ca", "los angeles, ca", "san diego, ca",
    "san jose, ca", "oakland, ca", "palo alto, ca", "mountain view, ca",
    "sunnyvale, ca", "santa clara, ca",
    # Other
    "austin, tx", "dallas, tx", "houston, tx", "san antonio, tx",
    "portland, or", "phoenix, az", "atlanta, ga",
...
EXTRACT_PROMPT = """\
Extract ONE Illinois job posting from the text below.
...
Return ONLY valid JSON in this exact format if a valid Illinois job with explicit USD salary range is found:
{{"role":"Job Title","company":"Company Name","min":80000,"max":120000,"location":"Chicago, IL","source_url":"{url}","posted":"YYYY-MM-DD"}}
...
- Not an Illinois location (Chicago, Evanston, Schaumburg, Naperville, remote IL, etc.)
...
- location must be in Illinois (Chicago, Evanston, Schaumburg, Naperville, Rockford, or remote roles mentioning IL pay range)
...
def extract_job(url, snippet, page_text, log=None):
    ...
    if not any(t in job.get("location", "").lower() for t in IL_TERMS):
        return None
