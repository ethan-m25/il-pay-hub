#!/usr/bin/env bash
# il-pay-hub/scripts/nightly-pipeline.sh
# Nightly pipeline for IL Pay Hub.
...
SCRIPTS_DIR="$HOME/il-pay-hub/scripts"
...
log "=== IL nightly pipeline started ==="
...
# === Phase 3 discovery — hub: il (added 2026-05-27) ===
log "--- Phase 3 discovery: hub_search_jobs ---"
python3 "$HOME/shared-scripts/hub_search_jobs.py" --hub "il" >> "$LOG_FILE" 2>&1 || true
log "hub_search_jobs done (exit $?)"
log "--- Phase 3 discovery: hub_deep_search ---"
python3 "$HOME/shared-scripts/hub_deep_search.py" --hub "il" >> "$LOG_FILE" 2>&1 || true
log "hub_deep_search done (exit $?)"
log "--- Phase 3 discovery: hub_search_kpmg ---"
python3 "$HOME/shared-scripts/hub_search_kpmg.py" --hub "il" >> "$LOG_FILE" 2>&1 || true
log "hub_search_kpmg done (exit $?)"
log "--- Phase 3 discovery: hub_search_sap ---"
python3 "$HOME/shared-scripts/hub_search_sap.py" --hub "il" >> "$LOG_FILE" 2>&1 || true
log "hub_search_sap done (exit $?)"
log "--- Phase 3 discovery: hub_search_scout ---"
python3 "$HOME/shared-scripts/hub_search_scout.py" --hub "il" >> "$LOG_FILE" 2>&1 || true
log "hub_search_scout done (exit $?)"

# === Phase 5 classify — hub: il (added 2026-05-28) ===
log "--- Phase 5 classify_step ---"
python3 "$HOME/shared-scripts/region_classifier/classify_step.py" \
    --hub "il" --no-llm >> "$LOG_FILE" 2>&1 || true
log "classify_step done (exit $?)"
# === Phase 5.5 publish gate — hub: il (added 2026-05-28) ===
...
GATE_RAW="$HOME/.openclaw/shared/il-jobs-raw-$(date +%Y-%m-%d).txt"
if [[ -f "$GATE_RAW" ]]; then
...
log "--- healthcheck ---"
python3 "$HOME/shared-scripts/hub_pipeline_healthcheck.py" --failure-only --hub "il" >> "$LOG_FILE" 2>&1
log "healthcheck done (exit $?)"

log "--- archive-jobs ---"
python3 "$HOME/shared-scripts/hub_archive_jobs.py" --hub "il" --limit 100 >> "$LOG_FILE" 2>&1
log "archive done (exit $?)"
log "--- skill-extract ---"
python3 "$HOME/shared-scripts/run_locked.py" /tmp/ollama-model.lock \
    python3 "$HOME/shared-scripts/hub_skill_extract.py" --hub "il" --limit 200 >> "$LOG_FILE" 2>&1
log "skill-extract done (exit $?)"
log "--- classify-new-jobs ---"
python3 "$HOME/shared-scripts/run_locked.py" /tmp/ollama-model.lock \
    python3 "$HOME/shared-scripts/hub_classify_new_jobs.py" --hub "il" >> "$LOG_FILE" 2>&1 || true
...
log "=== IL nightly pipeline complete ==="
...
python3 "$HOME/shared-scripts/hub_pipeline_healthcheck.py" --failure-only \
  --hub il --pipeline-exit-code "$PIPELINE_RC" || true
exit "$PIPELINE_RC"
